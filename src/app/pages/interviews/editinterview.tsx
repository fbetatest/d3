...
const EditInterview: FC = () => {
  ...
  useEffect(() => {
    ...
    if(interviewData.surveyorname.length && !list){
    let tempSurveyors = surveyorsList

    interviewData.surveyorname.map((name: string) => {
      console.log(name)
      tempSurveyors = tempSurveyors.filter((item) => item !== name)
    })
  ...
  }, [surveyorsList, interviewData.surveyorname.length])
  return (
    <>
        ...
        <div className='card-body py-3 pt-3 pb-3'>
          <div className='row g-5 gx-xxl-12'>
            <div className='col-xxl-12'>
              {interviewData.interviewName != 'Loading Data...' ? (
                <Formik
                  initialValues={{
                    interviewName: interviewData.interviewName,
                    projectName: interviewData.projectName,
                    surveyorname: interviewData.surveyorname,
                ...
                >
                  {(formik) => (
                    <Form>
                          ...
                          return (
                            ...
                            >
                              <label className='form-check form-check-sm form-check-custom form-check-solid me-5 mb-3'>
                                {interviewData.projectName == name ? (
                                  <input
                                    type='radio'
                                    name='projectName'
                                    value={name}
                      ...
                      <div role='group' aria-labelledby='my-radio-group mb-4'>
                        {interviewData.surveyorname.map((name, id) => {
                          return (
                            <label
                              key={id}
                              className='d-flex align-items-center justify-content-between cursor-pointer  '
